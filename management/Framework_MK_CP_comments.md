...
|What are the flow of events for each scenario associated with each use case?|| unclear. Part of 'data flows' or do we need a separate field to identify this explicitly? <span style="color:green">CP: Isn't this 'Basic Flow of Events'? However, currently there is one flow per use case, but the description talks about multiple scenarios per use case.</span>|
|What are the outstanding issues with this use case?|| **(to add - free text field)**|
|What is the source of this use case?|| **(to add - free text field)**|
|What standards/specifications exist related to this use case?|| **(to add - options based on specification listing)**|
|How has this use case evolved over time?|| captured via page history |
...
<span style="color:green">CP: What does 'Conformance' mean? If not "optional", the information must be provided?</span>
...
|<span style="background:yellow">Geographic scope|How large of an area an implementation of this use case affects|| **(to add - how should this be specified?)** Q: what concern (above) does this address? <span style="color:green">CP: Is this something like 'parcel', 'block', ..., 'city', 'region'?</span> |
|<span style="background:yellow">Temporal scope</span>|The temporal duration of the use case|| **(to add - how should this be specified?)** Q: what concern (above) does this address? <span style="color:green">CP: 'seconds', 'minutes', 'hours', ..., 'years'?</span> |
...
<span style="color:green">CP: Is this a complete list? The boundaries between some groups are not clear to me, but we probably do not need clear definitions and this is just a help to get started?</span>
...
<span style="color:green">CP: The wiki has classes, object properties, data properties, specifications, specification elements. My comments below look at where to find the topic in the wiki. I stopped after some entries, as resolving some comments above should resolve those, too.</span>

|Topic|Concern|Addressed by Model Kind|
|-----|-------|-----------------------|
Data Definition|What are the definitions of the major business terms?|Vocabulary <span style="color:green">CP: currently not part of the wiki?</span>|
Data Definition|How do business terms relate to one another?|Ontology <span style="color:green">CP: same</span>|
Data Definition|What does each data element mean?|Data Dictionary: Definition <span style="color:green">CP: data element = class, object property, data property?</span>|
Data Definition|How does each data element relate to other data elements? (including dynamic metadata)|Logical Data Model <span style="color:green">CP: can we derive the logical data model that is defined by the wiki entries?</span>|
Data Definition|Does the data have safety-of-life implications?|Data Dictionary: Usage <span style="color:green">CP: part of the description, if yes?</span>|
Data Definition|What, if any, state behavior relationships are there for data and classes?|State Machine <span style="color:green">CP: supplementary figures?</span>|
Data Definition|What is the representational form of the data within data exchanges? |Physical Data Model <span style="color:green">CP: ?</span>|
Data Definition|How is data transformed from one data exchange format into another?|Logical Data Model: Physical Representation <span style="color:green">CP: ?</span>|
Data Definition|How can the information view be extended to support additional domain or implementation-specific data?|Operations on Views <span style="color:green">CP: ?</span>|
